    ...
    <div style="background: #fafafa;">
        <div style="padding-top:40px;padding-bottom:53px; backgroung: #000">
            ...
            <div style="margin-bottom: 29px;">
                <mt-field class="tcth-field-required" v-for="(item,index) in extralist" :key="index" :label="'附件'+(index+1)" :placeholder="item" :disabled="true" type="number">
        ...
        <mt-popup v-model="popupVisibleExtra" position="right" class="mint-popup-car" :modal="false" popup-transition="popup-fade" style="background:#fafafa">
            <div style="height: 100%;overflow-y: scroll;">
                ...
                <div style="height:40px"></div>
                <div v-for="(item,index) in extralist" :key="index" style="margin-bottom:10px">
                    <mt-field :label="'附件'+(index+1)" placeholder="请输入价格名称" v-model="extralist[item]">
                        <mt-button v-if="index!=0" size="small" @click="removeNewPrice(index)">删除</mt-button>
                    </mt-field>
                        {{item}}
                </div>
